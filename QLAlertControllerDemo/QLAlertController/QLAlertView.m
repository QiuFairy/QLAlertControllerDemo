...
@interface QLAlertView()<UITableViewDelegate,UITableViewDataSource>
...
+ (instancetype _Nullable)alertViewWithFatherView:(UIView *)fatherView title:(nullable NSString *)title message:(nullable NSString *)message{
    QLAlertView *alertView = [[QLAlertView alloc] initWithFrame:fatherView.bounds];
...
- (void)addAction:(QLAlertAction *)action{
    ...
    action.propertyEvent = ^(QLAlertAction *action) {
        if (weakSelf.actions.count <= maxNumberOfActionHorizontalArrangementForAlert) {
            ...
            QLAlertTableViewCell *footerCell = weakSelf.footerCells[index];
...
- (void)layoutFooter {
    ...
    if ((self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert) && self.actions.count) { // 需要footerBezelView
        [footerBezelViewConstraints addObject:[NSLayoutConstraint constraintWithItem:footerBezelView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:QL_ActionHeight]];
    } else { // 不需要footerView
    ...
    if (footerCells.count && self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert) {
        [footerCells enumerateObjectsUsingBlock:^(QLAlertTableViewCell *footerCell, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (idx > 0) {
                ...
                // 分割线宽度
                [footerCellConstraints addObject:[NSLayoutConstraint constraintWithItem:line attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:QL_LineWidth]];
                // cell的右边距
...
- (CGFloat)actionBezelHeight:(CGFloat)footerTopMargin {
    ...
    if (self.actions.count) {
            if (self.actions.count <= _maxNumberOfActionHorizontalArrangementForAlert) {
                actionBezelHeight = QL_ActionHeight;
            } else {
...
- (CGFloat)minActionHeight:(CGFloat)footerTopMargin {
    ...
    if (self.cancelActions.count) {
        ...
        if (self.actions.count > 3) { // 没有取消按钮，其余按钮在3个或3个以上
            ...
            if (self.actions.count) {
                minActionHeight = QL_ActionHeight;
            }
        }
    }
